...
export function WaveformDisplay({ voiceName, waveformData, waveformDuration, currentTime, duration }: WaveformDisplayProps) {
  ...
  const draw = useCallback((canvas: HTMLCanvasElement) => {
    ...
    for (let x = 0; x < w; x++) {
      ...
      // Bounds check uses HTMLAudioElement duration (actual playback length)
      if (t < 0 || t > duration) continue;
      // Sample index uses waveformDuration (raw decoded length from AudioContext),
      // which may include encoder delay/padding that HTMLAudioElement strips.
      // This ensures the waveform scrolls in lockstep with the decoded audio data.
      const si = Math.floor((t / waveformDuration) * (samples - 1));
